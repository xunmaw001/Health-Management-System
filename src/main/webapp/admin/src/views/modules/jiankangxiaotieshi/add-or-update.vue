<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="来源" prop="laiyuan">
          <el-input v-model="ruleForm.laiyuan" 
              placeholder="来源" clearable  :readonly="ro.laiyuan"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.beijingtu" label="背景图" prop="beijingtu">
          <file-upload
          tip="点击上传背景图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.beijingtu?ruleForm.beijingtu:''"
          @change="beijingtuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.beijingtu" label="背景图" prop="beijingtu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.beijingtu.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fabushijian = true;
          ...
          if(o=='beijingtu'){
            this.ruleForm.beijingtu = obj[o];
	    this.ro.beijingtu = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `jiankangxiaotieshi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssm8twfe/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiankangxiaotieshi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiankangxiaotieshiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    beijingtuUploadChange(fileUrls) {
	this.ruleForm.beijingtu = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
