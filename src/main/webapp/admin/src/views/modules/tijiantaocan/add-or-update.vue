<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="套餐类型" prop="taocanleixing">
          <el-select v-model="ruleForm.taocanleixing" placeholder="请选择套餐类型">
            <el-option
                v-for="(item,index) in taocanleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="套餐类型" prop="taocanleixing">
	      <el-input v-model="ruleForm.taocanleixing"
                placeholder="套餐类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="体检项目" prop="tijianxiangmu">
          <el-select v-model="ruleForm.tijianxiangmu" placeholder="请选择体检项目">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="体检项目" prop="tijianxiangmu">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="体检地点" prop="tijiandidian">
          <el-input v-model="ruleForm.tijiandidian" 
              placeholder="体检地点" clearable  :readonly="ro.tijiandidian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="体检地点" prop="tijiandidian">
              <el-input v-model="ruleForm.tijiandidian" 
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="体检内容" prop="tijianneirong">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.tijianneirong" label="体检内容" prop="tijianneirong">
                    <span>{{ruleForm.tijianneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.taocanmingcheng = true;
          ...
          if(o=='taocanleixing'){
            this.ruleForm.taocanleixing = obj[o];
	    this.ro.taocanleixing = true;
            continue;
          }
          if(o=='tijianxiangmu'){
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../ssm8twfe/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tijiantaocan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tijiantaocanCrossAddOrUpdateFlag = false;
                  this.parent.search();
